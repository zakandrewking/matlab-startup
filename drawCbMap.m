...
for i = 1:length(map.text)
    ...
    %textFont
    if ~(isempty(find(regexp(textFont,' B')))) || ~isempty(find(regexp(textFont(end),'B'))) %BUGFIXEDBYZAK
        textWeight = 'bold';
...
for i = 1:size(map.rxnLabelPosition,2)
    if ~any(isnan(map.rxnLabelPosition(:,i)))
      ...
      if isfield(options, 'rxnTextSize')
        ...
        drawText(map.rxnLabelPosition(1,i),map.rxnLabelPosition(2,i), ...
                 map.connectionAbb{find(map.rxnIndex(i)==map.connection,1)},12,'italic'); %BUGFIXEDBYZAK
      end
    end
end
